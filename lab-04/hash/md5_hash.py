...
def md5(message):
    ...
    # Tính độ dài chuỗi văn bản
    original_length = len(message)
    message += b'\x80'  # Sử dụng bytes cho ký tự '\x80'
    while len(message) % 64 != 56:
        message += b'\x00'  # Dùng bytes cho '\x00'

    # Chuyển message thành dạng byte array
    message = list(message)

    # Chia chuỗi thành các block size=64
